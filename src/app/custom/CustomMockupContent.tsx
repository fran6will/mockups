...
import { UploadCloud, Sparkles, Loader2, Image as ImageIcon, CheckCircle, AlertCircle, Download, Wand2, LayoutTemplate, Palette, Plus, X, Lock } from 'lucide-react';
...
export default function CustomMockupContent() {
    ...
    useEffect(() => {
        const loadScene = async () => {
            ...
            setMode('remix');
            setTitle(`Remix of ${scene.title}`);
            setPrompt(scene.custom_prompt || '');

            // Fetch scene image and convert to File for style reference
            if (scene.base_image_url) {
                try {
                    const response = await fetch(getOptimizedSupabaseUrl(scene.base_image_url, 1000));
                    const blob = await response.blob();
                    const file = new File([blob], "scene-reference.png", { type: "image/png" });
                    setStyleReferences([file]);
                } catch (e) {
                    console.error("Failed to load scene image as reference:", e);
                }
            }
        };
    ...
    const getCreditCost = () => {
        if (imageSize === '2K') return 10;
        if (imageSize === '4K') return 15;
        return 5;
    };
            ...
            {successProduct ? (
                ...
                <div className="grid md:grid-cols-2 gap-12 items-start mb-20">
                    ...
                    <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-ink/5 space-y-6">
                        ...
                        <div>
                            <label className="block text-sm font-bold text-ink/70 mb-2">Image Quality</label>
                            <div className="flex bg-gray-50 p-1 rounded-xl border border-ink/10">
                                {['1K', '2K', '4K'].map((size) => {
                                    const isLocked = size === '4K' && !isPro; // Assuming isPro is defined in the component scope
                                    return (
                                        <button
                                            key={size}
                                            onClick={() => !isLocked && setImageSize(size as any)}
                                            disabled={isLocked}
                                            className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all flex flex-col items-center justify-center gap-0.5 ${imageSize === size
                                                ? (mode === 'template' ? 'bg-teal text-white shadow-sm' : 'bg-purple-600 text-white shadow-sm')
                                                : isLocked
                                                    ? 'text-ink/30 cursor-not-allowed bg-gray-100'
                                                    : 'text-ink/60 hover:bg-white/50'
                                                }`}
                                        >
                                            <span className="flex items-center gap-1">
                                                {size}
                                                {isLocked && <Lock size={10} />} {/* Assuming Lock icon is imported */}
                                            </span>
                                            <span className={`text-[10px] opacity-80 ${imageSize === size ? 'text-white' : 'text-ink/40'}`}>
                                                {size === '1K' ? '5 Credits' : size === '2K' ? '10 Credits' : '15 Credits'}
                                            </span>
                                        </button>
                                    );
                                })}
                        ...
                        >
                            {isGenerating ? (
                                ...
                                <>
                                    ...
                                    {mode === 'template' ? "Generate Template" : "Remix Product Shot"}
